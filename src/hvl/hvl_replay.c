...
*/
struct hvl_tune *hvl_load_hvl( const uint8 *buf, uint32 buflen, uint32 unused, uint32 freq )
{
  const uint8  *bptr;
  const TEXT   *nptr;
...
uint32 hvl_getSamplesPerFrame(struct hvl_tune *ht)  {
	return  ht->ht_Frequency/50;
}
